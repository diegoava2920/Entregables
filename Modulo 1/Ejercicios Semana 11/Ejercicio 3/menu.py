    ...
    while(opcion != "7"):#while para repetir la lista hasta que el resultado sea 7 (osea la opcion de cerrado)
        try:#try para tomar cualquier error que aparezca y enviar in mensaje dependiendo de cual sea
            ...
            if(int(opcion) == 1) :
                ...
                student_porfile = [ 'Nombre' , 'Seccion' ,'Espanol','Ingles','Sociales','Ciencias', 'Promedio']# template de informacion a pedir para agregar un estudiante
                data.write_csv_file("student_list.csv", actions.student_list, student_porfile)
